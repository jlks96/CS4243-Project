...
for character in characters:
    for component in components:
        source_folder = os.path.join(source_dir, character, component, "original")
        save_folder = os.path.join(save_dir, character, component)
        for path, subdirs, files in os.walk(source_folder):
            for filename in files:
                name = Path(filename)
                image_id = str(name.with_suffix(''))
                image_file = os.path.join(str(source_folder), str(filename))
